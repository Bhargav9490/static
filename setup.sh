...
plan() {
  ...
  echo "DNS:       ${DNS_ARR[*]}"
  echo
  echo "Will disable waits and managers:"
  echo "  - systemd-networkd-wait-online"
  echo "  - systemd-networkd (if present)"
  echo "  - NetworkManager (if present)"
...
apply_changes() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-get -y install ifupdown >/dev/null 2>&1 || true
